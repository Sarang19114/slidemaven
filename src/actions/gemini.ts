...
export const generateCreativePrompt = async (userPrompt: string) => {
  ...
  const finalPrompt = `
      Create a groundbreaking and innovative PowerPoint presentation in JSON format on the topic ${userPrompt} . The JSON output must strictly follow this structure:

{ "slides": [ { "slide_number": <integer>, "title": "<Concise Slide Title>", "subtitle": "<Optional Subtitle Summarizing the Slide>", "content": [ "<Bullet point sentence detailing a key aspect of the slide>", "<Another bullet point sentence if needed>" ], "visual_elements": [ "<Description of a visual element or diagram that enhances the message>", "<Additional visual details if applicable>" ], "speaker_notes": "<A sentence providing further context or commentary for the slide>" } // Additional slide objects... ] }

Your presentation must include at least eight slides covering the following general sections:

Introduction and Context:  Present the project idea, its purpose, and relevance.
Overview and Background: Provide essential context or background information.
Goals and Objectives: Outline what the project aims to achieve and why.
Key Features and Highlights: Describe the unique or creative aspects of the project.
Methodology or Approach: Explain the strategy or process for achieving the goals.
Benefits and Impact: Discuss the anticipated outcomes and overall significance.
Challenges and Considerations: Identify potential obstacles and critical factors.
Next Steps and Conclusion: Summarize the proposal and suggest a path forward.
Ensure each slide is logically structured with clear bullet points, descriptive visual element details, and speaker notes that enhance understanding. Output only valid JSON adhering exactly to the structure provided, with no additional text or commentary.
  `;
